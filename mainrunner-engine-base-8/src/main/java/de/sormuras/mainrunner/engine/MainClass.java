...
class MainClass extends AbstractTestDescriptor {
  ...
  static MainClass of(Class<?> mainClass, TestDescriptor parent) {
    UniqueId uniqueId = parent.getUniqueId().append("main-class", mainClass.getName());
    String displayName = mainClass.getName();
    ClassSource testSource = ClassSource.from(mainClass);
    MainClass result = new MainClass(uniqueId, displayName, testSource);
